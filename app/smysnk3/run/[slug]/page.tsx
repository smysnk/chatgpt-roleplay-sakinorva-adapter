...
import type { Smysnk3Analysis } from "@/lib/smysnk3Score";
import { normalizeSmysnk3OptionKey } from "@/lib/smysnk3Score";
import { getSmysnk3OptionRationalBullets } from "@/lib/smysnk3OptionRationale";
...
const getModeLabel = (mode: Smysnk3Mode) => {
  if (mode === 16) {
    return "16 (fast)";
  }
  if (mode === 64) {
...
const formatRunMode = (runMode: Smysnk3RunPayload["runMode"]) => {
  if (runMode === "ai") {
    return "AI roleplay";
  }
  if (runMode === "reddit") {
...
const buildHeatmapModel = (
  contexts: Smysnk3Analysis["turbulentMode"]["contexts"],
  selectedType: string
): HeatmapModel | null => {
  ...
  const expectedByArchetype = new Map(
    SMYSNK3_ARCHETYPE_ORDER.map((archetype, index) => [
      archetype,
      ([...stack, ...shadow][index] ?? stack[0]) as HeatmapFunctionKey
    ])
  );

  const hitsByArchetype = SMYSNK3_ARCHETYPE_ORDER.reduce(
    (acc, archetype) => {
    ...
    {} as Record<(typeof SMYSNK3_ARCHETYPE_ORDER)[number], Record<string, number>>
  );

  contexts.forEach((context) => {
    const byArchetype = new Map(context.archetypeBreakdown.map((row) => [row.archetype, row]));
    SMYSNK3_ARCHETYPE_ORDER.forEach((archetype) => {
  ...
  const maxHits = Math.max(
    0,
    ...SMYSNK3_ARCHETYPE_ORDER.flatMap((archetype) =>
      contextEntries.map((contextEntry) => hitsByArchetype[archetype][contextEntry.key] ?? 0)
    )
  );
  const rows: HeatmapRow[] = SMYSNK3_ARCHETYPE_ORDER.map((archetype) => ({
    archetype,
    archetypeLabel: SMYSNK3_ARCHETYPE_LABELS[archetype],
...
const getTurbulentTopType = (analysis: Smysnk3Analysis) => {
...
export default function Smysnk3RunPage({ params }: { params: { slug: string } }) {
  const [data, setData] = useState<Smysnk3RunPayload | null>(null);
  ...
  const [openRational, setOpenRational] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    ...
    const load = async (withLoading = false) => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/smysnk3/slug/${params.slug}`);
        if (!response.ok) {
          const payload = await response.json().catch(() => ({}));
          throw new Error(payload?.error ?? "Failed to load SMYSNK3 run.");
        }
        const payload = (await response.json()) as Smysnk3RunPayload;
        if (active) {
          setData(payload);
        }
      } catch (err) {
  ...
  const mode = useMemo(
    () => parseSmysnk3Mode(data?.questionMode ?? data?.questionCount ?? 32),
    [data?.questionMode, data?.questionCount]
  );

  const responseMap = useMemo(() => {
    if (!data?.responses?.length) {
      return new Map<string, { answer: Smysnk3OptionKey | null; rationale: string }>();
    }
    return new Map(
      data.responses.map((response) => [
        response.questionId,
        {
          answer: normalizeSmysnk3OptionKey(response.answer),
  ...
  const assertiveTypeMatching = useMemo(() => {
    ...
    const fromAssertive = (analysis.assertiveMode as { typeMatching?: Smysnk3Analysis["typeMatching"] } | undefined)
      ?.typeMatching;
  ...
  const assertiveArchetypeRows = useMemo(() => {
    ...
    const indexByArchetype = new Map(
      SMYSNK3_ARCHETYPE_ORDER.map((archetype, index) => [archetype, index])
    );
        ...
        <div className="app-card">
          ...
          {loading ? (
            ...
            <div style={{ marginTop: "20px" }}>
              ...
              <p className="helper">{`SMYSNK3 ${getModeLabel(mode)} | ${formatRunMode(data.runMode)}`}</p>
                ...
                <>
                  ...
                  {hasAnalysis ? (
                    <div style={{ marginTop: "24px", display: "grid", gap: "16px" }}>
                      <h3 style={{ marginBottom: 0 }}>SMYSNK3 scoring summary</h3>
                    ...
                    <p className="helper" style={{ marginTop: "24px" }}>
                      Detailed SMYSNK3 archetype confidence is unavailable for this run.
                    </p>
                  ) : null}
                </>
              )}
                  ...
                  <div className="answer-row" key={question.id}>
                    ...
                    {response?.answer ? (
                      <div style={{ display: "grid", gap: "10px", marginTop: "10px" }}>
                        {!openRational[question.id] ? (
                          <div>
                            <button
                              type="button"
                              className="button secondary"
                              onClick={() =>
                                setOpenRational((current) => ({
                                  ...current,
                                  [question.id]: true
                                }))
                              }
                          ...
                          <div className="app-card" style={{ padding: "12px 14px" }}>
                            {response.rationale ? (
                              <div style={{ display: "grid", gap: "6px", marginBottom: "10px" }}>
                                <p className="helper rationale-heading" style={{ margin: 0 }}>
                                  {(data?.subject?.trim() || "Character")}:
                                </p>
                                <p className="helper rationale-quote" style={{ margin: 0 }}>
                                  <span>"</span>
                            ...
                            {(() => {
                              const selectedOption = question.options.find(
                                (option) => option.key === response.answer
                              );
                              if (!selectedOption) {
                                return null;
                              }
                              const bullets = getSmysnk3OptionRationalBullets(
                                question,
                                selectedOption
                              );
                              return (
                                <div style={{ display: "grid", gap: "6px" }}>
                                  ...
                                  <ul className="helper" style={{ margin: 0, paddingLeft: "18px" }}>
                                    {bullets.map((bullet) => (
                                      <li key={`${question.id}-${selectedOption.key}-${bullet}`}>
