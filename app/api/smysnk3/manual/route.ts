...
export async function POST(request: Request) {
  try {
    ...
    const questionIds =
      ...
      hasBalancedSmysnk3QuestionIds(requestedIds, questionMode)
        ? requestedIds
        : null;
    const scenarios = getSmysnk3Scenarios(questionMode, questionIds, slug);
    ...
    const responses: { questionId: string; answer: Smysnk3OptionKey; rationale: string }[] = [];
    for (const response of payload.responses) {
      ...
      const answerKey = normalizeSmysnk3OptionKey(response.answer);
      if (!answerKey) {
