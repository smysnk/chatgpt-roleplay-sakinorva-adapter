...
const normalizeStoredResponses = (run: Run, allowedIds: Set<string>): StoredResponse[] => {
  ...
  rawResponses.forEach((response) => {
    ...
    const key = normalizeSmysnk2OptionKey(response.answer);
    if (!key) {
      return;
    }
    normalized.push({
...
export async function GET(
  ...
  const run = await Run.findOne({
    where: { slug: params.slug, indicator: "smysnk2", runMode: "user" }
  });
...
export async function PATCH(
  ...
  try {
    ...
    const answerKey = normalizeSmysnk2OptionKey(payload.answer);
    if (!answerKey) {
